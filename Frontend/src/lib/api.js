...
if (import.meta.env.DEV) {
  console.log('🔧 API Mode:', USE_MOCK ? 'MOCK (using mock API)' : 'REAL (connecting to backend)')
  if (USE_MOCK) {
...
if (USE_MOCK) {
  ...
  apiClient.get = async (url, config) => {
    ...
    try {
      ...
      } else if (endpoint.includes('/payroll/payruns/') && endpoint.endsWith('/preview')) {
  ...
  apiClient.post = async (url, data, config) => {
    ...
    try {
      ...
      } else if (endpoint.includes('/payroll/payruns/') && endpoint.endsWith('/process')) {
        const payrunId = endpoint.split('/')[3]
